...
UVXY_SPIKE_ABORT_PCT = 0.08

# ── Reprice v2: regime-gated chase + guaranteed descent (Jun 26 fix) ──
REPRICE_TIERS = {
    "CALM":    dict(target_min=180, hard_min=210, chase=True,  chase_cap0=1.00),
...
TAKE_BID_OPEN_DELAY_MIN = 15    # opening-mid rescue trigger activates after this many min
...
def _reprice_target(regime, mid0, live_mid, live_bid, live_ask, elapsed_min, uo, un):
   ...
   # +8% live UVXY move vs placement baseline → abort (Phase 2/3 only)
...
class TradierClient:
    ...
    def _retry_5xx(self, fn, *args, **kwargs):
    ...
    def has_working_order(self, option_symbol: str) -> bool:
...
def place_with_reprice(client, underlying: str, option_symbol: str, side: str,
    ...
    prev_mid = mid0          # previous-cycle mid for the take-bid pop trigger
    ...
    while _market_open():
        ...
        # ── re-quote the live option (freshness-guarded) ──
        try:
...
def _is_short_entry_day(today) -> bool:
...
def run(sandbox: bool = True, preview: bool = False, check_only: bool = False):
    ...
    actions_taken = []
    is_short_day  = _is_short_entry_day(today)  # Mon/Fri, or the trading day before a holiday Mon/Fri
                ...
                for _cand_exp in _dte_candidates:
                    ...
                    try:
                        ...
                        if not _best:
                            ...
                            continue
                        _liq_ok, _liq_reason = check_liquidity(_best["bid"], _best["ask"], _best.get("open_interest"))
                        if _liq_ok:
                            exp  = _cand_exp
                            best = _best
                ...
                try:
                    ...
                    if not preview:
                        result = place_with_reprice(
                            ...
                            "buy_to_open", 1,
                            best["bid"], best["ask"], phase_label=phase)
                            ...
                            if best_new:
                                ...
                                if not preview:
                                    ...
                                    r_buy = place_with_reprice(
                                        ...
                                        best_new["bid"], best_new["ask"], phase_label=phase)
...
LOCK_PATH = os.path.expanduser("~/vix_suite/tradier_orchestrator.lock")
...
def main():
    ...
    # ── single-instance lock (order-placing modes only) ──────────────────────
