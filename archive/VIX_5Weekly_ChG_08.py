...
def backtest_vix_5pct(
    ...
    """
    Backtest with three strategy modes:
    - Static (fixed params)
    - Static 90%-only
    - Dynamic (percentile-based) using meta-rules if available
    """
                ...
                for tgt in target_grid:
                    for sig in sigma_grid:
                        res = backtest_vix_5pct(
                            ...
                            meta_rules_df=None,  # static scan
                        )
                        rows.append(
                            {
                                "entry_pct": ep,
...
st.write(
    "Simulate the VIX 5% Weekly Diagonal (LEAP + Weekly Covered Calls) on historical VIX data.\n"
    "- Long-dated ATM calls with weekly short OTM calls\n"
    "- Per-contract fees\n"
    "- Strategy modes: static / 90%-only / dynamic percentile-based\n"
    "- Grid scan with meta-rule extraction and upload"
)
...
with st.sidebar:
    ...
    long_dte_weeks = st.slider(
        "Long call maturity (weeks) [static baseline]",
        4,
        104,
    ...
    otm_points = st.slider(
        "Short call OTM distance (pts) [baseline]",
        1.0,
        10.0,
    ...
    # Grid ranges
    grid_exp = st.expander("Grid scan: parameter ranges (incl. DTE)", expanded=False)
    with grid_exp:
        entry_grid_str = st.text_input(
            "Entry percentiles (decimals, e.g. 0.10, 0.30, 0.50, 0.90)",
            value=st.session_state.get("grid_entry_list", "0.05, 0.10, 0.30, 0.50, 0.70, 0.90"),
            key="grid_entry_list",
        )
        otm_grid_str = st.text_input(
            "OTM distances (points, e.g. 3, 5, 7)",
            value=st.session_state.get("grid_otm_list", "8,9,10,11,12,13,14,15,16"),
            key="grid_otm_list",
        )
        target_grid_str = st.text_input(
            "Target multiples (e.g. 1.10, 1.20, 1.30)",
            value=st.session_state.get("grid_target_list", "1.15"),
            key="grid_target_list",
        )
        sigma_grid_str = st.text_input(
            "Sigma multipliers (e.g. 0.8, 1.0, 1.2)",
            value=st.session_state.get("grid_sigma_list", "0.3"),
            key="grid_sigma_list",
        )
        dte_grid_str = st.text_input(
            "Long DTE weeks (e.g. 3, 13, 26)",
            value=st.session_state.get("grid_dte_list", "3"),
            key="grid_dte_list",
        )
...
with strategy_expander:
    meta_flag = "meta-rules from last grid scan" if meta_rules_df is not None and not meta_rules_df.empty else "built-in default schedule"
    st.markdown(
        f"""
**Strategy name:** VIX 5% Weekly Diagonal (LEAP + Weekly Covered Calls)  
**Strategy mode:** `{strategy_mode}`  

- Weekly VIX data from **{start_date}** to **{end_date}**, 52-week rolling percentile.
- Static mode: enter when VIX below **{entry_percentile*100:.1f}%** of last {entry_lookback_weeks} weeks.
- Static 90%-only: enter only if VIX percentile ≥ 90%.
- Dynamic mode: parameters follow a schedule based on current VIX percentile, using **{meta_flag}**.
...
with playbook_simple:
    ...
    if "Entry mode" in execution_mode:
        st.markdown(
            f"""
**You selected:** `Entry mode` (you assume no long VIX call is currently open)

1. **Once per week (your chosen execution day):**
   - Check current **VIX** on broker.
   - App shows latest 52w percentile ≈ **{pct_str}**.

2. **Entry condition (current mode: `{strategy_mode}`):**
...
5. **Exit long:**
   ...
   - Time-to-expiry is very small (near zero).

6. **After exit:**
   - Record PnL and go back to **Entry mode** (this screen).
"""
        )
    else:
        st.markdown(
            f"""
**You selected:** `Management mode` (you already have a long VIX call open)

1. **This week: manage the existing position**
   - You already hold a long VIX LEAP-style call.
   - Focus on **selling / managing the weekly short call** and **checking exit** on the long.

2. **Weekly short call overlay:**
   - On your execution day, check current VIX.
   - For **each long contract**, sell **1 × VIX call**:
     - Expiry: ~1 week
     - Strike ≈ **spot + {otm_points:.1f} pts**
   - Use your broker's quotes to pick an actual tradable strike.

3. **Monitor during the week:**
   - Let the short decay toward expiry, or
   - Consider closing early if:
     - Most of the premium is captured, or
     - VIX spikes well through the strike.

4. **Check exit trigger for long call:**
   - Compare current mid price vs **entry price × {target_multiple:.2f}**.
   - Also consider time remaining to expiry:
     - If almost no time is left, consider exiting regardless.

5. **If long is closed:**
   - You exit the entire diagonal (both long and short).
   - Next week, switch your **Execution mode** back to `Entry` and follow the entry checklist.
"""
        )
    ...
    try:
        ...
        if spot > 0 and T_long > 0:
            ...
            if "Entry mode" in execution_mode:
                st.markdown(
                    f"""
**Execution mode:** `Entry` (no long open, template for a *new* diagonal)

If your **entry conditions are met today** according to the app:

- **Underlying:** VIX options (on VIX futures)
- **Current VIX (spot):** {spot:.2f}  
- **Mode:** `{strategy_mode}`  

**Suggested long leg (LEAP-like):**

- Buy **{contracts_suggested} × VIX call**  
- Approx expiry: **{long_dte_weeks} weeks** from now  
- Strike: **ATM ≈ {spot:.0f}**  
- Model premium per contract ≈ **${price_long*100.0:,.2f}**  
- Delta ≈ **{delta_long:.2f}**

**Risk / sizing:**

- Portfolio equity ≈ **${ending_equity:,.0f}**  
- Allocation = **{alloc_pct*100:.2f}%** → capital at risk ≈ **${risk_dollars:,.0f}**  
- Per-contract cost incl. fees ≈ **${per_contract_cost:,.2f}**  
- Suggested contracts ≈ **{contracts_suggested}**

**Short weekly call overlay (for next weeks once long is open):**

- Each week, sell **1 × VIX call** per long contract  
- Expiry: ~1 week  
- Strike ≈ **spot + {otm_points:.1f} ≈ {short_strike:.1f}**

**Profit target idea (for the long):**

- Exit when long call price ≥ **{target_multiple:.2f} × entry price** (or when very near expiry).
"""
                )
            else:
                st.markdown(
                    f"""
**Execution mode:** `Management` (you already hold a long VIX call)

This section assumes you **already opened** your LEAP-like VIX call earlier.

**Weekly short call idea (per long contract):**
...
- Let this short call decay into expiry, or
- Consider closing early if:
  - Most of the premium is captured, or
  - VIX spikes well past the strike and risk is uncomfortable.

Separately, check your **long call**:

- Compare current long-call price to **entry_price × {target_multiple:.2f}**.
- If price ≥ target OR time-to-expiry is very small, consider closing the long and the whole structure.
"""
                )
        else:
...
# ----- Grid scan & meta-rules & upload -----

show_grid = st.checkbox("Show parameter grid scan, meta-rules & XLSX tools", value=False)
...
if show_grid:
    ...
    st.markdown("### Upload existing scan (XLSX) to reuse meta-rules")
    ...
    if uploaded_file is not None:
        try:
            ...
            if required_cols.issubset(set(up_df.columns)):
                ...
                st.success("Uploaded scan loaded. Meta-rules updated for Dynamic mode.")
            else:
    ...
    if grid_df is None or grid_df.empty:
        ...
        # Meta-rules
        st.subheader("Meta-rules learned from grid (per entry_pct)")
...
st.caption(
    ...
    "with static / 90%-only / dynamic percentile-based modes, grid scan, trade durations, "
