...
def parse_int_list(text: str, default_vals):
    """Parse a comma-separated list of ints, fallback to default_vals on error/empty."""
    try:
        parts = [p.strip() for p in text.split(",")]
...
def backtest_vix_5pct(
    ...
    - trade_count = number of long-call entries.
    - durations tracked in weeks (bars) for each completed trade.
    """
    ...
    trade_count = 0        # count how many times we open a long
    current_trade_open_idx = None  # index when current trade opened
    durations = []         # list of trade durations in weeks (bars)
    ...
    for i in range(n - 1):
                ...
                if S <= threshold and S > 0:
                    ...
                    if price_long > 0:
                        ...
                        if contracts > 0:
                            ...
                            trade_count += 1  # count this long-call entry
                            current_trade_open_idx = i  # mark open index
        ...
        if has_long and long_contracts > 0:
            ...
            if price_short > 0:
                ...
                # Open short: cash up, realized when settled
                cash += short_premium
                cash -= open_short_fee
            ...
            if exit_now:
                ...
                # record duration (bars) for this trade
...
# ---------- Grid scan (includes DTE, trades, durations) ----------
...
# Apply saved defaults (if triggered)
...
with st.sidebar:
    ...
    # ----- Grid scan entry/exit setup (ranges, including DTE) -----
    grid_exp = st.expander("Grid scan: parameter ranges (incl. DTE)", expanded=False)
    with grid_exp:
        entry_grid_str = st.text_input(
            "Entry percentiles (decimals, e.g. 0.10, 0.20, 0.30)",
            value=st.session_state.get("grid_entry_list", "0.10, 0.20, 0.30, 0.50, 0.70, 0.90"),
            key="grid_entry_list",
        )
        otm_grid_str = st.text_input(
            "OTM distances (points, e.g. 2, 3, 5)",
            value=st.session_state.get("grid_otm_list", "2, 3, 5"),
            key="grid_otm_list",
        )
        target_grid_str = st.text_input(
            "Target multiples (e.g. 1.10, 1.20, 1.25, 1.50, 2.0)",
            value=st.session_state.get("grid_target_list", "1.10, 1.20, 1.25, 1.50, 2.0"),
            key="grid_target_list",
        )
        sigma_grid_str = st.text_input(
            "Sigma multipliers (e.g. 0.8, 1.0, 1.2)",
            value=st.session_state.get("grid_sigma_list", "0.8, 1.0, 1.2"),
            key="grid_sigma_list",
        )
        dte_grid_str = st.text_input(
            "Long DTE weeks (e.g. 13, 26, 39, 52)",
            value=st.session_state.get("grid_dte_list", "13, 26, 39, 52"),
            key="grid_dte_list",
        )

# Parse grid ranges
entry_grid_vals = parse_float_list(
    st.session_state.get("grid_entry_list", "0.10, 0.20, 0.30"), [0.10, 0.20, 0.30]
)
otm_grid_vals = parse_float_list(
    st.session_state.get("grid_otm_list", "2, 3, 5"), [2.0, 3.0, 5.0]
)
target_grid_vals = parse_float_list(
    st.session_state.get("grid_target_list", "1.10, 1.25, 1.50"), [1.10, 1.25, 1.50]
)
sigma_grid_vals = parse_float_list(
    st.session_state.get("grid_sigma_list", "0.8, 1.0, 1.2"), [0.8, 1.0, 1.2]
)
dte_grid_vals = parse_int_list(
    st.session_state.get("grid_dte_list", "13, 26, 39, 52"), [13, 26, 39, 52]
)

entry_lookback_weeks = 52  # used both in backtest and percentile
...
# ----- Compute rolling VIX percentile (52w) -----

# Make sure we have a 1-D float array
vix_vals = np.asarray(vix_weekly, dtype=float).reshape(-1)

# 1-D array of same length, filled with NaN
vix_pct = np.full(len(vix_vals), np.nan, dtype=float)

for i in range(len(vix_vals)):
    if i >= entry_lookback_weeks - 1:
        lookback_window = vix_vals[i - entry_lookback_weeks + 1 : i + 1]
        current_vix = vix_vals[i]
        vix_pct[i] = np.mean(lookback_window <= current_vix)
...
1. **Data & timeframe**  
   ...
   - 52-week rolling window used for regime percentile.

2. **Regime filter**  
   - Each week, look back **{entry_lookback_weeks} weeks** of VIX closes.  
...
3. **Long VIX call entry**  
   ...
   - Risk per trade ≤ **{alloc_pct*100:.2f}%** of current equity (premium + estimated fees).
...
7. **Equity curve & metrics**  
   ...
   - **Total number of long-call trades opened:** {trade_count}  
   - **Average duration of a completed trade:** {avg_trade_duration:.2f} weeks  
   - **Min / Max trade duration:** {min_trade_duration:.0f} – {max_trade_duration:.0f} weeks
"""
    )
...
# ----- Charts: Equity+VIX (dark) & Weekly PnL+Percentile (dark), side by side -----
...
# ----- Grid scan section (results persist; DTE + trades + durations; XLSX; refresh) -----
...
if show_grid:
    ...
    with col_clear:
        clear_scan_clicked = st.button("Refresh / Clear scan results")

    # Clear previous scan from session_state without reloading the app
    if clear_scan_clicked:
        st.session_state.grid_results_df = None
        st.session_state.last_grid_idx = 0
        if "grid_choice_params" in st.session_state:
            del st.session_state["grid_choice_params"]
        st.session_state.apply_from_grid = False
    ...
    # ... inside the "if show_grid" block, after grid_df exists ...
    ...
    with col_gen:
        if st.button("Generate XLSX from current grid"):
            ...
            buffer.seek(0)
            # store raw bytes in session_state so they survive reruns
            st.session_state["grid_xlsx_bytes"] = buffer.getvalue()

    with col_dl:
        if "grid_xlsx_bytes" in st.session_state:
            st.download_button(
        ...
        if sel_idx != st.session_state.get("last_grid_idx", 0):
            st.session_state["grid_choice_params"] = {
                ...
                "long_dte_weeks": float(grid_df.loc[sel_idx, "long_dte_weeks"]),
            }
...
st.caption(
    ...
    "with Black-Scholes pricing, fixed per-contract fees, grid scanning (incl. DTE, trade count, and trade durations), "
    "user-selectable parameter presets, VIX percentile display, and separated realized vs unrealized PnL."
)
