...
def black_scholes_call(S: float, K: float, T: float, r: float, sigma: float) -> float:
    """Simple Black-Scholes call pricer for VIX options (theoretical)."""
    if T <= 0.0:
        return max(S - K, 0.0)
    if sigma <= 0.0:
...
# =========================
# Realistic VIX option model
# =========================

def _realism_haircut_factor(pct: float, realism_level: str = "Normal") -> float:
    """
    Crude 'haircut' to turn ideal BS prices into something more like real VIX options.
    - Stronger haircut in calm regimes (contango bleed).
    - Softer haircut in high-vol regimes (backwardation/spikes).
    pct is VIX 52w percentile in [0,1] (nan -> 0.5).
    Returns factor in [0.2, 0.9].
    """
    if np.isnan(pct):
        pct = 0.5
    ...
    # Calm -> heavier haircut; high vol -> lighter haircut
...
def price_vix_call(
    ...
    - If realistic_mode=False: pure BS on spot.
    - If realistic_mode=True:
        * Apply 'haircut' to mimic futures-based pricing, contango bleed,
          IV crush, skew & spreads in aggregate.
    """
    bs_price = black_scholes_call(S_spot, K, T, r, sigma_base)
    if not realistic_mode or T <= 0:
        return bs_price

    factor = _realism_haircut_factor(vix_pct, realism_level=realism_level)
    return bs_price * factor


# =========================
# Meta-rule extractor (for diagonal)
# =========================
...
# =========================
# Dynamic schedules (for diagonal dynamic)
# =========================

def param_schedules_default(pctl: float) -> dict:
    """
    Fallback mapping from VIX percentile (0–1) to parameters (diagonal).
    """

    long_dte_weeks = int(np.interp(pctl, [0.0, 0.5, 1.0], [52, 13, 3]))
    otm_points = float(np.interp(pctl, [0.0, 1.0], [3.0, 7.0]))
    target_multiple = float(np.interp(pctl, [0.0, 1.0], [2.0, 1.15]))
    sigma_mult = float(np.interp(pctl, [0.0, 1.0], [1.0, 0.8]))
...
def run_grid_scan_long_only(
    ...
    """
    Grid scan for LONG-ONLY VIX call strategy.
    Varies:
        - exit_mode
        - entry_pct
        - long_dte_weeks
    ...
    grid_df["score"] = grid_df.apply(score_row, axis=1)
    sorted_df = grid_df.sort_values(["score", "cagr"], ascending=[False, False])
    best_row = sorted_df.iloc[0]
    return sorted_df, best_row
...
st.write(
    ...
    "on historical VIX data, with realistic VIX options mode and grid scans."
)
...
if "long_grid_results_df" not in st.session_state:
    st.session_state.long_grid_results_df = None
...
with st.sidebar:
    ...
    realistic_mode = st.checkbox(
        "Realistic VIX options mode (futures / IV crush / spreads)",
        value=True,
        help="If ON, long VIX options are haircut to mimic futures-based pricing, "
             "contango bleed, IV crush, and spreads. If OFF, pure Black-Scholes on spot VIX.",
        key="realistic_mode_flag",
    )

    realism_level = st.selectbox(
        "Realism strength",
    ...
    # Grid ranges
    grid_exp = st.expander("Grid scan: parameter ranges (Diagonal & Long-only)", expanded=False)
    with grid_exp:
        ...
        otm_grid_str = st.text_input(
            "OTM distances (points, e.g. 3, 5, 7, 10, 14) [Diagonal only]",
        ...
        if "Long-only" in structure_mode:
            exit_modes_str = st.text_input(
                "Long-only exit modes to test (comma; TP only / TP + stop / Percentile exit)",
                value=st.session_state.get("grid_long_exit_modes", "TP only, TP + stop"),
                key="grid_long_exit_modes",
            )
        else:
            exit_modes_str = st.session_state.get("grid_long_exit_modes", "TP only, TP + stop")
...
raw_exit = st.session_state.get("grid_long_exit_modes", "TP only, TP + stop")
exit_tokens = [t.strip() for t in raw_exit.split(",") if t.strip() != ""]
valid_exit_names = {"TP only", "TP + stop", "Percentile exit"}
exit_modes_grid = [e for e in exit_tokens if e in valid_exit_names] or ["TP only"]
...
with strategy_expander:
    ...
    realism_desc = "ON (realistic VIX futures / IV crush / spreads)" if realistic_mode else "OFF (pure BS on spot VIX)"

    st.markdown(
        f"""
**Structure:** {struct_label}  
**Entry mode:** `{strategy_mode}`  
**Realistic pricing:** {realism_desc}{extra_exit_text}
...
1. **Once per week (your execution day):**
   - Check **VIX** on broker.
...
1. Weekly: check VIX and percentile (≈ {pct_str}).

2. Entry condition (mode: `{strategy_mode}`) with no weekly short overlay.

3. When entry condition is met:
   - Equity ≈ **${ending_equity:,.0f}**; risk ≈ **${ending_equity*alloc_pct:,.0f}** (at {alloc_pct*100:.2f}%).
   - Buy ATM **VIX call** with ≈ **{long_dte_weeks} weeks** to expiry.

4. Exit long (method: `{exit_mode}`):
...
5. After exit: back to Entry mode.
"""
            )
    else:
        if "Diagonal" in structure_mode:
...
1. Manage existing diagonal this week.
...
3. Monitor & roll:
   - Let short decay or close early if most premium captured or VIX spikes.

4. Long exit:
   - Exit if long price ≥ **{target_multiple:.2f}× entry** or near expiry.
"""
            )
        else:
            st.markdown(
...
2. Each week:
   - Check if `{exit_mode}` exit is triggered:
     - TP: price ≥ **{target_multiple:.2f}× entry**.
     - Stop: price ≤ **{stop_mult:.2f}× entry** (if TP + stop).
     - Percentile: percentile ≤ **{exit_pct_threshold*100:.1f}%** (if Percentile exit).
    ...
    try:
        ...
        if spot > 0 and T_long > 0:
            ...
            price_long = price_vix_call(
                ...
                T=T_long,
                r=risk_free,
                sigma_base=sigma_long_guess,
                vix_pct=current_vix_pct if not np.isnan(current_vix_pct) else 0.5,
                realistic_mode=realistic_mode,
                realism_level=realism_level,
            )
            per_contract_cost = price_long * 100.0 + 2.0 * fee_per_contract
...
if show_grid:
    ...
    if "Long-only" in structure_mode:
        ...
        with col_run:
            run_scan_clicked = st.button("Run / Update DIAGONAL grid scan")
        with col_clear:
            clear_scan_clicked = st.button("Clear DIAGONAL scan + meta-rules")
        ...
        if grid_df is None or grid_df.empty:
            st.info("No DIAGONAL grid scan results yet. Run a scan or upload a previous XLSX.")
        else:
            st.markdown(
            ...
            # XLSX export (robust)
            st.markdown("### Export DIAGONAL scan results")
            col_gen, col_dl = st.columns(2)
            with col_gen:
                if st.button("Generate XLSX from current DIAGONAL grid"):
                    try:
                        buffer = io.BytesIO()
            ...
            with col_dl:
                if st.session_state.grid_xlsx_bytes:
                    st.download_button(
                        label="Download DIAGONAL scan as XLSX",
                        data=st.session_state.grid_xlsx_bytes,
                        file_name="vix_5pct_grid_scan_diagonal.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        key="download_vix_5pct_grid_scan_diag",
                    )
                else:
                    st.info("Click **Generate XLSX from current DIAGONAL grid** first.")
...
show_long_grid = st.checkbox("Show LONG-ONLY grid scan (exit method test + interpretation)", value=False)

if show_long_grid:
    st.subheader("Long-Only Parameter Grid Scan (with Exit Methods)")

    if "Diagonal" in structure_mode:
        st.info("To run the LONG-ONLY grid scan, switch 'Position structure' to 'Long-only: VIX Call (no weekly shorts)'.")
    else:
        col_run_l, col_clear_l = st.columns(2)
        with col_run_l:
            run_long_scan_clicked = st.button("Run / Update LONG-ONLY grid scan")
        with col_clear_l:
            clear_long_scan_clicked = st.button("Clear LONG-ONLY scan")

        if clear_long_scan_clicked:
            st.session_state.long_grid_results_df = None

        if run_long_scan_clicked:
            long_df, long_best = run_grid_scan_long_only(
                vix_weekly=vix_weekly,
                vix_percentile_series=vix_percentile_series,
                initial_capital=initial_capital,
                alloc_pct=alloc_pct,
            ...
            st.session_state.long_grid_results_df = long_df

        long_grid_df = st.session_state.long_grid_results_df
        if long_grid_df is None or long_grid_df.empty:
            st.info("No LONG-ONLY grid scan yet. Set ranges in the sidebar and click the scan button.")
        else:
            st.markdown(
                "Sorted by **score = CAGR - 2×|MaxDD|**, then by higher CAGR."
            )
            st.dataframe(long_grid_df.round(4), use_container_width=True)

            # Exit-mode summary
            summary = (
                long_grid_df.groupby("exit_mode")[["cagr", "max_dd", "sharpe", "trades"]]
                .mean()
                .sort_values("cagr", ascending=False)
            )
            st.markdown("**Average performance per exit method (LONG-ONLY):**")
            st.dataframe(summary.round(4))

            # --- Interpretation panel ---
            interp = st.expander("AI Interpretation of LONG-ONLY grid scan", expanded=True)
            with interp:
                try:
                    # Best row by score
                    best_row = long_grid_df.sort_values("score", ascending=False).iloc[0]
                    best_exit = best_row["exit_mode"]
                    best_cagr = best_row["cagr"]
                    best_dd = best_row["max_dd"]
                    best_ep = best_row["entry_pct"]
                    best_dte = best_row["long_dte_weeks"]
                    best_tgt = best_row["target_mult"]
                    best_sig = best_row["sigma_mult"]

                    # Best CAGR row
                    best_cagr_row = long_grid_df.sort_values("cagr", ascending=False).iloc[0]
                    best_cagr_exit = best_cagr_row["exit_mode"]
                    best_cagr_only = best_cagr_row["cagr"]
                    best_cagr_dd = best_cagr_row["max_dd"]

                    # Per-exit-mode best score
                    idx_per_exit = long_grid_df.groupby("exit_mode")["score"].idxmax()
                    per_exit_best = long_grid_df.loc[idx_per_exit].copy()

                    st.markdown(
                        f"""
### 🔍 High-level conclusions

- **Realistic mode:** `{"ON" if realistic_mode else "OFF"}` (`{realism_level}`)  
- **Combinations tested:** **{len(long_grid_df)}**  

**Overall best combo by score (CAGR - 2×|MaxDD|):**

- Exit mode: **{best_exit}**  
- Entry percentile: **{best_ep:.2f}**  
- DTE (weeks): **{best_dte:.0f}**  
- Target multiple: **{best_tgt:.2f}**  
- Sigma multiplier: **{best_sig:.2f}**  
- CAGR: **{best_cagr*100:.1f}%**  
- MaxDD: **{best_dd*100:.1f}%**

**Best pure CAGR combo (ignoring drawdown):**

- Exit mode: **{best_cagr_exit}**  
- CAGR: **{best_cagr_only*100:.1f}%**  
- MaxDD: **{best_cagr_dd*100:.1f}%**
"""
                    )

                    st.markdown("### 📊 Best per exit method")
                    st.dataframe(
                        per_exit_best[
                            ["exit_mode", "entry_pct", "long_dte_weeks", "target_mult", "sigma_mult", "cagr", "max_dd", "sharpe", "trades"]
                        ].round(4),
                        use_container_width=True,
                    )

                    # Some qualitative text
                    top_exit_by_cagr = summary.index[0]
                    st.markdown(
                        f"""
### 🧠 Interpretation (LONG-ONLY as research lens)

- **Best average exit style by CAGR:** `{top_exit_by_cagr}`  
- Long-only with realistic pricing **does not replace** the diagonal; it shows:
  - Which **entry percentiles** are survivable.
  - Which **DTE** and **target multiples** avoid total bleed.
  - How much the **exit rule** changes drawdown vs CAGR.

- In practice you can treat LONG-ONLY as:
  - A **tail-risk / crash hedge benchmark**, and
  - A way to tune diagonal parameters (entry_pct, DTE, target, sigma_mult).

- Your production trading structure should still be:
  - **Diagonal** = LEAP + Weekly short calls
  - **Long-only** = diagnostic, not primary engine.
"""
                    )

                    if not realistic_mode:
                        st.warning(
                            "Realistic mode is OFF. Results may be overly optimistic (no contango / IV crush / spreads). "
                            "For trading decisions, keep Realistic mode ON."
                        )
                except Exception as e:
...
st.caption(
    ...
    "and a Long-only VIX call strategy, with realistic pricing haircuts, static / 90%-only / dynamic modes, "
    "DIAGONAL & LONG-ONLY grid scans, trade durations, meta-rules, XLSX export, and interpretive summaries."
)