"""
Patch: Upgrade daily_signal.py email to Risk & Structure Dashboard.
- Paper email: blue theme, paper watermark, signals-focused
- Real email: orange/red theme, LIVE CAPITAL banner, risk-first
- Both: delta, IV ratio, roll debit estimates, stress test, scaling block
"""
...
# ══════════════════════════════════════════════════════════════════════
# 1. Add helper functions before build_position_aware_email
# ══════════════════════════════════════════════════════════════════════
...
# Insert helpers before build_position_aware_email
marker = "def build_position_aware_email("
if "_bs_delta" not in src:
    src = src.replace(marker, helper_funcs + marker)
    print("✓ Added email helper functions")
else:
    print("  Helpers already present")

# ══════════════════════════════════════════════════════════════════════
# 2. Replace build_position_aware_email with upgraded version
# ══════════════════════════════════════════════════════════════════════

# Find the function boundaries
start_marker = "def build_position_aware_email("
end_marker   = "\ndef send_email("
...
if start_idx < 0:
    print("✗ build_position_aware_email not found")
    exit(1)

old_func = src[start_idx:end_idx]

new_func = '''def build_position_aware_email(
    batch,
    variant_states,
) -> str:
    """
...
"""
    for pos in sorted(all_pos, key=lambda p: p.variant_id):
        ...
        if short_dte <= 0:    urgency, uc = "🔔 ACTION REQUIRED", "#cc0000"
...
if old_func in src:
    ...
    print("✓ Replaced build_position_aware_email with upgraded version")
    print("✓ Added build_real_capital_email for live capital")
else:
    print("✗ Could not find old function to replace")

# ══════════════════════════════════════════════════════════════════════
# 3. Update main() to also send real capital email
# ══════════════════════════════════════════════════════════════════════

old_main_send = '''    html = build_position_aware_email(batch, variant_states)

    # 6. Determine subject'''

new_main_send = '''    html = build_position_aware_email(batch, variant_states)

    # 5b. Build and send real capital email (separate)
    try:
        ...
        if real_html:
            ...
            success_real, msg_real = send_email(real_html, real_subject, real_html)
            if success_real:
                print(f"   ✅ Real capital email sent")
            else:
                print(f"   ⚠️ Real capital email failed: {msg_real}")
    except Exception as e:
        print(f"   ⚠️ Real capital email error: {e}")

    # 6. Determine subject'''

if old_main_send in src:
    src = src.replace(old_main_send, new_main_send)
    print("✓ main() updated to send separate real capital email")
else:
    print("✗ main() send block not found")

# ══════════════════════════════════════════════════════════════════════
# 4. Update paper email subject line to add [PAPER] prefix
# ══════════════════════════════════════════════════════════════════════

old_subj = '''    subject = f"[VIX 5%] {regime_state.regime.value.upper()} ({percentile:.0%}) — "
    subject += f"{len(management)} Open, {len(recommended)} Entry, {len(paper_test)} Observe"'''
new_subj = '''    subject = f"[PAPER 📋] {regime_state.regime.value.upper()} ({percentile:.0%}) — "
    subject += f"{len(management)} Open, {len(recommended)} Entry, {len(paper_test)} Observe"'''

if old_subj in src:
    src = src.replace(old_subj, new_subj)
    print("✓ Paper email subject prefixed with [PAPER 📋]")

path.write_text(src)
print("\n✅ Patch complete — restart vix_app.service and test with: python3 daily_signal.py --dry-run")
