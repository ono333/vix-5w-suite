...
MAX_EP_DAYS = 60
MIN_GAP_DAYS = 10      # min days between episode starts (dedupe clustered triggers)
...
def find_episodes(df):
    ...
    for i, d in enumerate(dates):
        if not trigger.iloc[i] or i + 2 >= len(dates):
            continue
        if last_start is not None and (i - last_start) < MIN_GAP_DAYS:
            continue
        ...
        if peak_px <= entry_px * 1.02:   # dud, no harvestable move
            continue
...
def main():
    ...
    if not episodes:
        sys.exit("No episodes — loosen SPIKE_TRIG")
    atr = atr_series(df)
    ...
    print("\nEpisode dates:")
    for ep in episodes:
        print(f"  {df.index[ep['start']].date()} -> {df.index[ep['end']].date()}"
